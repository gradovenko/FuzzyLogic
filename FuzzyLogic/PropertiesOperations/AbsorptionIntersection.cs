...
namespace FuzzyLogic.PropertiesOperations
{
    public class AbsorptionIntersection : IBinaryProperty
    {
        ...
        {
            var rightBrackets = new Operation().Intersection(set1, set2).Result;
            return new Operation().Union(set1, rightBrackets).Result.CurrentSet.SequenceEqual(set1.CurrentSet);
        }
    }
}
